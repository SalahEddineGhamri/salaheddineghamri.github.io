        ...
        if let Some(dir_name) = option_dir_name {
            if dir_name.starts_with("post") {
                ...
                let image = document
                    ...
                    .next()
                    .map(|n| n.attr("content").unwrap_or_default())
                    .unwrap_or_default()
                    .to_string();
                let description = document
                    ...
                    .to_string();
                let date = document
                    .find(Attr("name", "publish-date"))
                    .next()
                    .map(|n| n.attr("content").unwrap_or_default())
                    .unwrap_or_default()
                    .to_string();

                let tags_vector = document
                    .find(Class("post-tags").descendant(Name("a")))
                    .map(|tag_item| tag_item.text())
                    .collect::<Vec<String>>();
                ...
                let path = format!("/posts/{}/", dir_name);
                let post = post::Post::new(0, title, description, image, date, path, tags);
                posts.push(post);
            }
        }
    }
    all_tags.insert(0, "all".to_string());
    (posts, all_tags)
}
...
pub fn filter_posts(tag_name: &str, posts: Vec<post::Post>) -> Vec<post::Post> {
    ...
    posts
        .into_iter()
        .filter(|post| post.has_tag(tag_name))
        .collect::<Vec<_>>()
}

#[function_component]
pub fn Blog() -> Html {
    let tag = use_state(|| "all".to_string());
    ...
    let listed_tags: Vec<Html> = all_tags.iter().map(|tag_item: &String| html!{ <> <tag::Tag tag_name={tag_item.clone()} href={"/#/blog".to_string()} on_tag_click={on_tag_click.clone()}/> </>}).collect();
